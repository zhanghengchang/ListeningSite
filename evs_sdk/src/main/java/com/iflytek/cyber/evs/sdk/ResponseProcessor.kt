                ...
                name.startsWith(Constant.NAMESPACE_LAUNCHER) -> {
                    launcher?.let {
                        when (name) {
                            ...
                            Launcher.NAME_SELECT -> {
                                val number = payload.getIntValue(Launcher.KEY_NUMBER)
                                it.select(number, object : Launcher.ExecuteCallback() {
                                    override fun onSuccess(feedbackText: String?) {
                                        it.sendSelectSucceed(feedbackText)
                                    }
                                    ...
                                    override fun onFailed(
                                        ...
                                        it.sendSelectFailed(failureCode, feedbackText)
                                    }
                                })
                            }
                        }
                ...
                name.startsWith(Constant.NAMESPACE_RECOGNIZER) -> {
                    recognizer?.let {
                        when (name) {
                            ...
                            Recognizer.NAME_VISUAL_RESULT -> {
                                it.getVisualCallback()?.onVisualResult(payload.toString())
                            }
                            Recognizer.NAME_TRANS_RESULT -> {
                                it.getVisualCallback()?.onTransResult(payload.toString())
                            }
                            else -> {
                            }
                        }
