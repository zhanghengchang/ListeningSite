...
abstract class Launcher {
    ...
    companion object {
        ...
        const val NAME_START_INTERNAL_APP_RESULT =
            "${Constant.NAMESPACE_LAUNCHER}.start_internal_app_result"
        const val NAME_SELECT = "${Constant.NAMESPACE_LAUNCHER}.select"
        const val NAME_SELECT_RESULT = "${Constant.NAMESPACE_LAUNCHER}.select_result"
        ...
        const val KEY_PAGE_INFO = "page_info"
        const val KEY_SELECTABLE = "selectable"
        ...
        const val FAILURE_CODE_INVALID_NUMBER = "INVALID_NUMBER"
        const val FAILURE_NONSELECTABLE_PAGE = "NONSELECTABLE_PAGE"
    }
    ...
    abstract fun select(number: Int, callback: ExecuteCallback)
    ...
    open fun sendSelectSucceed(feedbackText: String?) {
        val payload = JSONObject()
        payload[PAYLOAD_RESULT] = RESULT_SUCCEED
        feedbackText?.let {
            payload[PAYLOAD_FEEDBACK_TEXT] = it
        }
        RequestManager.sendRequest(NAME_SELECT_RESULT, payload)
    }

    open fun sendSelectFailed(failureCode: String?, feedbackText: String?) {
        val payload = JSONObject()
        payload[PAYLOAD_RESULT] = RESULT_FAILED
        payload[PAYLOAD_FAILURE_CODE] = failureCode
        feedbackText?.let {
            payload[PAYLOAD_FEEDBACK_TEXT] = it
        }
        RequestManager.sendRequest(NAME_SELECT_RESULT, payload)
