...
import com.boll.audiobook.hear.network.response.MyWorkListBean;
...
public class MyWorkPlayActivity extends BaseActivity implements View.OnClickListener,
    ...
    private static final String TAG = "MyWorkPlayActivity";
    ...
    private List<MyWorkListBean> mWorkListBeans;
...
//                    int diff = TimeUtil.diffToMin(timeDiff);
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        if (playMode == 1) {
            ...
            int randNumber = rand.nextInt(mWorkListBeans.size());
            position = randNumber;
        }
        Log.e(TAG, "position: " + position);
        if (position >= mWorkListBeans.size()) {
            position = 0;
        }
        playAudio(position);
    }
    ...
    public int initLayout() {
        return R.layout.activity_my_wrok_play;
    }
    ...
    public void initView() {
        ...
        iconPic = findViewById(R.id.icon_pic);
    ...
    public void initData() {
        ...
        mWorkListBeans = (List<MyWorkListBean>) intent.getSerializableExtra("audioList");
        initIfNecessary();
        ...
        if (mWorkListBeans != null && mWorkListBeans.size() > 0) {
            ...
            for (int i = 0; i < mWorkListBeans.size(); i++) {
                ...
                localAudioBean.setTitle(mWorkListBeans.get(i).getName());
    ...
    public void playAudio(int position) {
        ...
        for (int i = 0; i < mWorkListBeans.size(); i++) {
            if (i == position) {
                picUrl = mWorkListBeans.get(i).getPicUrl();
                title = mWorkListBeans.get(i).getName();
                audioUrl = mWorkListBeans.get(i).getAudioUrl();
            }
        }
    ...
    public void playLeft() {
        position--;
        if (position < 0) {
            position = mWorkListBeans.size() - 1;
        }
        playAudio(position);
    }
